...
export const CuratorTeam = ({ team, stantion }: Props) => {
  ...
  const [answerAccepted, setAnswerAccepted] = React.useState(
    Boolean(localStorage.getItem(`${team.team_name}-end-time`))
  );

  useEffect(() => {
    // если команда уже стартовала, или закончила, но баллы еще не отправлены
    const startTime = localStorage.getItem(`${team.team_name}-start-time`);

    if (answerAccepted) {
      const endTime = localStorage.getItem(`${team.team_name}-end-time`);
      forceUpdateTimer(
        Number(endTime) - Number(startTime),
        stantion.time * 60 * 1000
      );
  ...
  const handleStart = () => {
    ...
    localStorage.setItem(`${team.team_name}-start-time`, String(time));
      ...
      <BriefArticle
        ...
        FooterContent={
          <div className={b('controls')}>
            ...
            <div className={b('timer', { 'too-much': isTimeEnded })}>
              {spentTime}
            </div>
      ...
      <BriefArticle
        ...
        FooterContent={
          <div className={b('fines')}>
            <FinePlate
              title="Курение на станции"
              points={3}
              updatePoints={setCurrentPoints}
            />
            <FinePlate
              title="Мат на станции"
              points={2}
              updatePoints={setCurrentPoints}
            />
            <FinePlate
    ...
    return (
      <div className={fb()}>
        ...
        <div className={fb('text-wrapper')}>
          ...
          >{`-${points} ${plural(['балл', 'балла', 'баллов'], points)}`}</div>
        </div>
